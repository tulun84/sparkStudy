...
/**
  ...
  *
  */
object ConsumerUpdateStateByKeyWC {
  ...
  def main(args: Array[String]) {
    ...
    ssc.checkpoint("hdfs://nameservice1/user/root/checkpoint/wordcount") //设置有状态的检查点

    //wordCounts.print(3)  //每个批次打印3行

    /**
      * updateStateByKey()方法传给updateFunc函数的参数说明:
      *  1.当前批次内，当前key的所有value组成seq，即currValues,
      *  2.之前批次中当前key的value总和，即preValueState
      */
    val addFunc=(currValues:Seq[Int],preValueState:Option[Int])=>{
      //计算即currValues序列中的所有value的和
      val currCount = currValues.sum
      val preCount = preValueState.getOrElse(0) //首个批次的preValueState=0
      Some(currCount+preCount)
    }
