...
object ClassificationPipeline {
  def main(args: Array[String]) {
    /*if (args.length < 1){
    println("Usage:lClassificationPipeine inputDataFile")
    sys.exit(1)
    }*/
    val conf = new SparkConf().setAppName("Classification with ML Pipeline").setMaster("local[8]")
    ...
    /** Step 1
      ...
      * ... ...
      */
    val filePath = "src/main/testFiles/data_banknote_authentication.txt"

    val parsedRDD = sc.textFile(filePath).map(_.split(",")).map(eachRow => {
